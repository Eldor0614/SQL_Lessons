...
SELECT COUNT(*) AS CN_ROW FROM  Customers;
...
SELECT SUM(SaleAmount) AS TOTAL_SALE FROM Sales WHERE ProductID = 7;
...
SELECT SUM (SaleAmount) AS TOTAL_SALE FROM Sales GROUP BY CustomerID;




SELECT DepartmentName, COUNT(*) AS TOTAL FROM Employees GROUP BY DepartmentName HAVING COUNT (*) > 5;




SELECT ProductID, SUM(SaleAmount) AS TOTAL_SALE, AVG(SaleAmount)AS AVG_SALE FROM Sales GROUP BY ProductID;



SELECT COUNT (*) AS HR_EMPLYEEES FROM Employees WHERE DepartmentName = 'HR';
...
SELECT 
CustomerID, 
SUM(TotalAmount) AS TotalSale FROM Orders
GROUP BY CustomerID 
...
SELECT 
custid, 
SUM(CASE WHEN freight > 50 THEN Freight ELSE 0 end ) AS total_above50, 
...
SELECT * FROM PRODUCTS
 ...
 COUNT(DISTINCT ProductID) AS UQ_Products FROM Orders 
