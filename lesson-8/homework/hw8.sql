...
SELECT 
Category, 
count(StockQuantity) AS TotalQuantity 
FROM Products 
GROUP BY Category;
...
--10. Using Orders and Invoices tables, 
--list customer IDs who placed orders in 2023 January to find those who did not have invoices.
...
SELECT YEAR(OrderDate) AS Year,
	AVG(TotalAmount) AS AnnualSale
	FROM Orders
	GROUP BY YEAR(OrderDate)
	ORDER BY Year

--14. Using Products table, group products based on price: 'Low' (<100),
-- 'Mid' (100-500), 'High' (>500). Return productname and pricegroup.
...
--15. Using City_Population table, use Pivot to show values of Year column in seperate columns ([2012],
-- [2013]) and copy results to a new Population_Each_Year table.
...
) AS PV_city_population
PIVOT
(SUM(population) FOR year IN ([2012], [2013])) AS PVT_TABLE;

---THIS IS A WAY TO COPY THE RESULTS OF PIVOT TABLE INTO A NEW ONE 
...
(SELECT  
	...
	population,
	YEAR
	FROM City_Population) AS pvt_fisrtTable
	PIVOT
	(SUM(POPULATION) FOR District_name IN ([Bektemir], [Chilonzor], [Yakkasaroy])) AS PVT_table;
