...
SELECT 
CONCAT(EMPLOYEE_ID, '-', FIRST_NAME, ' ', LAST_NAME) AS FULL_Info
FROM Employees;
...
--3. Display the first name and the length of the first name for all employees whose 
...
SELECT 
Year1,
GREATEST(Max1, Max2, Max3) AS MaxResult
FROM TestMax;
...
SELECT
id,
COALESCE(ssn, passportid, itin ) as first_nonul_value
FROM person;
...
SELECT  
STRING_AGG(STRING, ',') within GROUP (ORDER BY SequenceNumber )AS GroupConcatination 
from DMLTable;
...
WHERE TEMP.FULL_NAME LIKE '%a%a%a%';
...
SELECT 
T1.DEPARTMENT_ID, 
T1.EmpCount_IN_EachDepartment, 
COUNT(T2.EMPLOYEE_ID) AS EmpWith3Years,
CAST(COUNT(T2.DEPARTMENT_ID) AS DECIMAL (10,2)) * 100.0 / T1.EmpCount_IN_EachDepartment AS PercentageWith3Years
FROM 
(
SELECT 
DEPARTMENT_ID, 
COUNT(EMPLOYEE_ID) AS EmpCount_IN_EachDepartment
...
WHERE  DATEDIFF(YEAR, HIRE_DATE, GETDATE())  > 3) AS T2
...
FullName, 
Grade, 
SUM(Grade) OVER (ORDER BY StudentID ASC ) AS Cumulative_Sum
FROM Students
...
--16. You have a table with two players (Player A and Player B) and their scores. 
--If a pair of players have multiple entries, aggregate their scores into a single row for
--each unique pair of players. Write an SQL query to calculate the total score for 
--each unique player pair(PlayerScores)
...
SELECT 
  CASE 
    WHEN PlayerA > PlayerB THEN PlayerB ELSE PlayerA END AS PlayerB,
  CASE 
    WHEN PlayerA > PlayerB THEN PlayerA ELSE PlayerB END AS PlayerA,
