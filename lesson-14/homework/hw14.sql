--1. Write a SQL query to split the Name column by a comma 
-- into two separate columns: Name and Surname.(TestMultipleColumns)
...
SELECT 
id, 
LEFT(Vals, CHARINDEX('.', Vals) - 1) AS First_Character, 
SUBSTRING(Vals, CHARINDEX('.', Vals)+ 1, CHARINDEX('.', Vals)- 1) AS Second_Character,
	CASE 
		WHEN LEN(Vals) >=5  THEN SUBSTRING(Vals, 5, 1)
		ELSE NULL
	END AS Third_Character
FROM Splitter
...
(5,'a.b.b.b.b.b..b..b'),
(6,'6.')
DECLARE @CharToCount ChAR(1) = '.';
SELECT
ID,
Vals,
(LEN(Vals) - LEN(REPLACE(Vals, @CharToCount, '')))CharacterCount
FROM testDots
WHERE (LEN(Vals) - LEN(REPLACE(Vals, @CharToCount, ''))) > 2;
...
--8. write a SQL query to find all dates' Ids with higher temperature 
...
RecordDate, 
Temperature, 
LAG(Temperature, 1) OVER (ORDER BY RecordDate) AS PreviousDay
FROM weather) AS TEMP
WHERE Temperature > TEMP.PreviousDay;
...
DECLARE @FRUIT_LISTS VARCHAR(100) = 'apple,banana,orange,grape'

SELECT  
*
FROM STRING_SPLIT(@FRUIT_LISTS, ',')
ORDER BY (SELECT NULL)
OFFSET 2 ROWS 
FETCH NEXT 1 ROW ONLY;

--11. Write an SQL query to determine the Employment Stage for each employee based on their HIRE_DATE. 
...
--12. Write a SQL query to extract the integer value that appears at the start of the string 
-- in a column named Vals.(GetIntegers)
...
SUBSTRING(VALS, 
PATINDEX('%[0-9]%', VALS), 
PATINDEX('%[0-9]%', SUBSTRING(VALS, PATINDEX('%[0-9]%', VALS), 8000)+ 'X')
) AS FIRST_INT
FROM GetIntegers;
...
Ordinal
FROM MultipleVals
CROSS APPLY  string_split([Vals], ',', 1)
)
 
 SELECT 
 Id, 
 STRING_AGG(Value, ',') WITHIN GROUP (ORDER BY 
 CASE 
	WHEN ORDINAL = 1 THEN 2
	WHEN ORDINAL = 2 THEN 1
	ELSE ORDINAL 
 END )AS SWAPPED_TABLE 
 FROM split_vals
 GROUP BY id;
...
CREATE TABLE Long_String (String VARCHAR(50))
INSERT INTO Long_String 
VALUES 
('sdgfhsdgfhs@121313131')


DECLARE @Long_String NVARCHAR(MAX) = 'sdgfhsdgfhs@121313131';

WITH CHARACTER AS (
SELECT 
	SUBSTRING (@Long_String, 1, 1) AS CHARACTER, 
	1 AS POSITION 
		UNION ALL
	SELECT 
	SUBSTRING (@Long_String, Position+1, 1 ) AS CHARACTER, 
	POSITION + 1
FROM CHARACTER
WHERE POSITION < LEN (@Long_String)
	)
SELECT CHARACTER
FROM CHARACTER;
...
JOIN
(SELECT B.Player_id,   
MIN(B.event_date) AS FIRST_LOGIN
FROM Activity AS B
GROUP BY Player_id
) AS TEMP_TABLE
ON  A.event_date = TEMP_TABLE.FIRST_LOGIN;

--16. Write a SQL query to separate the integer values and 
-- the character values into two different columns.(rtcfvty34redt)
...
SELECT STRING_AGG (CASE WHEN RESULT LIKE '[0-9]' THEN RESULT ELSE '' END, '') AS INT, 
STRING_AGG (CASE WHEN RESULT LIKE '[a-z]' THEN RESULT ELSE '' END, '' ) AS LETTERS
FROM CTE;
